...
class MusicPlayerDatabaseImpl(context: Context) : SQLiteOpenHelper(context, "musicPlayerDatabase.db", null, 1), MusicPlayerDatabase{
    ...
    override fun addPlaylist(playlistName: String, songIds: List<Long>) {
    ...
    override fun removePlaylist(playlistName: String) {
        writableDatabase.delete("playlist", "playlistName = ?", arrayOf(playlistName))
    }

    override fun getPlaylist(playlistName: String): List<PlaylistEntry> {
    ...
    override fun getAllPlaylistsGrouped(): Map<String, List<Long>> {
        val db = readableDatabase
        val cursor = db.query(
            "playlist",
            arrayOf("playlistName", "songId"),
